...
export class CategoriasService {
  ...
  createCategoria(
    ...
    // Agregar campos del formulario con validación mejorada
    Object.entries(data).forEach(([key, value]) => {
      if (value !== null && value !== undefined) {
        // Para campos booleanos, convertir explícitamente
        if (typeof value === 'boolean') {
          formData.append(key, value ? '1' : '0');
        }
        // Para strings, solo agregar si no están vacíos
    ...
    return this.http
      .post<CategoriaSingleResponse>(`${this.adminApiUrl}/categorias`, formData)
      .pipe(
        tap(() => {
          this.loadingSubject.next(false);
  ...
  /**
   * Actualizar categoría existente usando POST con soporte para imagen
   */
  updateCategoria(
    id: number,
    data: CategoriaFormData,
    ...
    return this.http
      .post<CategoriaSingleResponse>(
        `${this.adminApiUrl}/categorias/${id}/update`,
        formData
      )
      .pipe(
        tap(() => {
  ...
  /**
   * Subir/actualizar imagen específica de una categoría
   */
  uploadCategoriaImage(
    ...
    return this.http
      .post<CategoriaSingleResponse>(
        `${this.adminApiUrl}/categorias/${id}/imagen`,
        formData
      )
      .pipe(
        tap(() => {
   ...
   */
  removeCategoriaImage(id: number): Observable<CategoriaSingleResponse> {
    this.loadingSubject.next(true);

    return this.http
      .delete<CategoriaSingleResponse>(
        `${this.adminApiUrl}/categorias/${id}/imagen`
      )
      .pipe(
        tap(() => {
  ...
  private handleError = (error: any): Observable<never> => {
    ...
    if (error.status === 422 && error.error?.errors) {
      ...
      errorMessage =
        'Conflicto: La categoría no se puede eliminar porque tiene productos o subcategorías asociadas.';
    } else if (error.status === 413) {
      errorMessage = 'El archivo de imagen es demasiado grande.';
    } else if (error.status === 415) {
      errorMessage =
        'Tipo de archivo no soportado. Use formatos de imagen válidos.';
