...
export class ImagenProductoService {
  ...
  private readonly baseUrl = `${environment.apiUrl}/admin/imagenes-producto`;
  ...
  /**
   * Actualizar imagen existente usando POST method
   */
  updateImagen(
    id: number,
    ...
    return this.http
      .post<ImagenProductoResponse>(`${this.baseUrl}/${id}/update`, formData)
      .pipe(
        map((response) => {
          if (response.success && response.data) {
            // Actualizar lista local
  ...
  /**
   * Actualizar orden de múltiples imágenes usando POST method
   */
  updateOrder(request: UpdateOrderRequest): Observable<void> {
    ...
    return this.http
      .post<ApiResponse>(`${this.baseUrl}/update-order`, request)
      .pipe(
        map((response) => {
          if (response.success) {
