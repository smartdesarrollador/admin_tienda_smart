...
export class VariacionProductoService {
  ...
  private readonly baseUrl = `${environment.apiUrl}/admin/variaciones-producto`;
  ...
  filterByActivo(activo: boolean | null): void {
    const filters = { ...this.filters(), activo, page: 1 };
    this.loadVariaciones(filters);
  }
  ...
  filterByStock(conStock: boolean | null): void {
    const filters = { ...this.filters(), con_stock: conStock, page: 1 };
    this.loadVariaciones(filters);
  }
